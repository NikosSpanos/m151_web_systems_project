...
def duration_predictor(logger_object:logging.Logger):
    ...
    model_artifacts_child:str = config.get("ml-settings", "cost_model_artifact")
    ...
    def read_parquet_files_in_chunks(parquet_files: List[str], chunk_size: int = 500_000):
        schema:dict = {
            'trip_cost': pl.Float64, #target
            
            'trip_duration': pl.Float64, #1
            
            'pickup_daytime': pl.Int64,
            'dropoff_daytime': pl.Int64,
            
            'pickup_hour': pl.Int8,
            'dropoff_hour': pl.Int8,

            'pickup_weekday': pl.Int8,
            'dropoff_weekday': pl.Int8,
            
            'pickup_quarter': pl.Int8, #7
            'dropoff_quarter': pl.Int8, #8
            
            'pickup_seconds': pl.Int64, #9
            'dropoff_seconds': pl.Int64, #10
            
            'pickup_holiday': pl.UInt8,
            'dropoff_holiday': pl.UInt8,
            
            'pickup_weekend': pl.UInt8,
            'dropoff_weekend': pl.UInt8,
            
            'haversine_centroid_distance': pl.Float64 #15
        }
    ...
    for iteration, chunk_df in enumerate(read_parquet_files_in_chunks(parquet_directories, chunk_size), 1):
        ...
        # SET THE TATGET FEATURE
        y_features:str = "trip_cost"
        # FILTER OUT OUTLIERS BASED ON PERCENTILES OF TRIP_DURATION
        lowest_quantile = 0.05
        highest_quantile = 0.99
        percentile_bottom_bound = chunk_df.select(y_features).quantile(lowest_quantile).collect().item()
        percentile_top_bound = chunk_df.select(y_features).quantile(highest_quantile).collect().item()
        rounded_bottom_bound = np.floor(percentile_bottom_bound)
        chunk_df = chunk_df.filter(pl.col(y_features) >= rounded_bottom_bound)
        chunk_df = chunk_df.filter(pl.col(y_features) <= percentile_top_bound)
        ...
        target_column_std = chunk_df.collect().describe().to_dict()[y_features][3]
...
if __name__ == "__main__":
    project_folder = "cost_recommendation_model"
